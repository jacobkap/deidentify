...
test_that("Errors are working as expected", {
  expect_error(deidentify_text("cat"), "key must be a cyphr key")
  expect_error(deidentify_text("cat",cols_to_encrypt = "t"),"Key must be provided")
  expect_error(deidentify_text("cat","1"),"key must be a cyphr key")
  expect_error(deidentify_text(data.frame("dog"),gen_aes_key()),"cols_to_encrypt is missing")
  expect_error(deidentify_text(data.frame(c1 = "dog"),gen_aes_key(),"c"),"You have selected")
})
...
test_that("Check to work that things are working as expected", {
  ...
  expect_equal(deidentify_text("cat", gen_aes_key(12)) != deidentify_text("cat", gen_aes_key(24)), TRUE)
  # data.frame checks
